    ...
    switch(method){
        case 'login':
            ...
            if (config && config.poolServer && config.poolServer.addressSeparator) {
              addresSepatator = config.poolServer.addressSeparator;
            }
            var splittedLogin = login.split(addresSepatator);
            ...
            var difficultyCandidate = parseInt(splittedLogin[splittedLogin.length-1],10);
            if (splittedLogin.length > 1 && difficultyCandidate === splittedLogin[splittedLogin.length-1]) {
               // Last element is int! It is difficulty!
            ...
            if (addressBase58Prefix !== cnUtil.address_decode(Buffer.from(splittedLogin[0]))){
            ...
            //collect login back
            login = splittedLogin.join(addresSepatator);
