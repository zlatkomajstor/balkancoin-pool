    ...
    async.waterfall([
        ...
        function(balances, callback){
            ...
            for (var worker in payments){
                if (worker.indexOf(addresSepatator) != -1) {
                  workerWithPaymentId.push(worker);
                  continue;
                }
            ...
            for (var worker of workerWithPaymentId) {
              var workerSplitted = worker.split(addresSepatator);
              var amount = parseInt(payments[worker]);
              transferCommands.push({
                ...
                rpc: {
                  destinations: [{
                    amount: amount,
                    address: workerSplitted[0]
                  }],
                  payment_id: workerSplitted[1],
                  fee: config.payments.transferFee,
                  mixin: config.payments.mixin,
                  unlock_time: 0
                } 
